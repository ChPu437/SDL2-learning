...
struct FC_Font {
	...
	// Codepoints are little endian (reversed from UTF-8) so that something like 0x00000005 is ASCII 5 and the map can
	// be indexed by ASCII values
	FC_Map* glyphs;
...
static void FC_Init(FC_Font* font) {
	...
	// Give a little offset for when filtering/mipmaps are used.  Depending on mipmap level, this will still not be
	// enough.
...
static FC_GlyphData* FC_PackGlyphData(FC_Font* font, Uint32 codepoint, Uint16 width, Uint16 maxWidth,
									  Uint16 maxHeight) {
...

	{
		...
		for (; *source_string != '\0'; source_string = U8_next(source_string)) {
			...
			if (!packed) {
				...
				// Upload the current surface to the glyph cache now so we can keep the cache level packing cursor up to
				// date as we go.
...
#endif
				// Update the glyph cursor to the new cache level.  We need to do this here because the actual cache
				// lags behind our use of the packing above.
...
#else
Uint8 FC_LoadFont(FC_Font* font, FC_Target* renderer, const char* filename_ttf, Uint32 pointSize, SDL_Color color,
				  int style)
#endif
{
	SDL_RWops* rwops;
...
static void FC_RenderAlign(FC_Font* font, FC_Target* dest, float x, float y, int width, FC_Scale scale,
						   FC_AlignEnum align, const char* text) {
	switch (align) {
	case FC_ALIGN_LEFT:
